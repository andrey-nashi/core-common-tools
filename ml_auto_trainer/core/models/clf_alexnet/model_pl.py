...
class AlexNet_Light(pl.LightningModule):

    def __init__(self, out_classes: int = 1, is_trained: bool = True, activation: str = None, loss_func: callable = None):
        super().__init__()

        # ---- Force pytorch lighting to ignore saving loss function, and other flags
        self.save_hyperparameters(ignore=["loss_func"])

        self.model = AlexNet(num_classes=out_classes, is_trained=is_trained, activation=activation)
