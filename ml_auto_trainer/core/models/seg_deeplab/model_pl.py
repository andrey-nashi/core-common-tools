...
class DeeplabV3_Light(pl.LightningModule):

    def __init__(self, in_channels: int = 3, out_classes: int = 1, is_trained: bool = True, backbone: str = DeeplabV3.BACKBONE_RESNET50, loss_func: callable = None):
        super().__init__()

        # ---- Force pytorch lighting to ignore saving loss function, and other flags
        self.save_hyperparameters(ignore=["loss_func"])

        self.model = DeeplabV3(num_classes=out_classes, in_channels=in_channels, is_trained=is_trained, backbone=backbone)
